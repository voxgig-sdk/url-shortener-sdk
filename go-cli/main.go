// Command UrlShortener-cli is a CLI + REPL for the UrlShortener SDK,
...
const prompt = "url-shortener"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "index"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      URL_SHORTENER_APIKEY (api key), URL_SHORTENER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: URL_SHORTENER_APIKEY carries the API key and
	// URL_SHORTENER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("URL_SHORTENER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("URL_SHORTENER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUrlShortenerSDK(opts)
