...
boru-driven command-line client **and** interactive REPL for the UrlShortener
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/url-shortener-cli)
make build
...
./url-shortener-cli load 1 index            # {id:1} shorthand
./url-shortener-cli load '{id:1}' index       # explicit match map

# 5. Override the API base URL for a single call
URL_SHORTENER_BASE=https://api.example.com ./url-shortener-cli load 1 index

# 6. No arguments -> interactive REPL
./url-shortener-cli
url-shortener> load 1 index
url-shortener> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/url-shortener-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/url-shortener-cli load 1 index
   ```
...
```sh
./url-shortener-cli load 1 index          # scalar shorthand for {id:1}
./url-shortener-cli load '{id:1}' index     # explicit match map
```
...
./url-shortener-cli load 1 index
```
...
Run with no arguments to open a REPL (prompt `url-shortener>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `index`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/url-shortener-cli`. |
...
(`./url-shortener-cli <expr>`) and interactively (the REPL), and expressions compose the
