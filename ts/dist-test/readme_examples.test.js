...
const __1 = require("..");
const SDK_NAME = 'UrlShortenerSDK';
...
const TEST_SEED = { "entity": { "index": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+UrlShortenerSDK\b/.test(code) ||
        /\bUrlShortenerSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+UrlShortenerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/UrlShortenerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
