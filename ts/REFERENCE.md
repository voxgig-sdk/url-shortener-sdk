# UrlShortener TypeScript SDK Reference

Complete API reference for the UrlShortener TypeScript SDK.
...
```ts
new UrlShortenerSDK(options?: object)
```
...
#### `UrlShortenerSDK.test(testopts?, sdkopts?)`
...
**Returns:** `UrlShortenerSDK` instance in test mode.
...
Alias for `UrlShortenerSDK.test()`.
...
```ts
const result = await client.Index().load({ url: 'url' })
```
...
Return the parent `UrlShortenerSDK` instance.
