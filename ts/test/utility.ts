...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.URLSHORTENER_TEST_LIVE ||
    'TRUE' === process.env.URLSHORTENER_TEST_OVERRIDE
  ) {
  ...
  m.URLSHORTENER_TEST_EXPLAIN = process.env.URLSHORTENER_TEST_EXPLAIN || m.URLSHORTENER_TEST_EXPLAIN
