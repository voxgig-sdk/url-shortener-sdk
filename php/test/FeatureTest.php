...
class FtCtrl extends UrlShortenerControl
{
...
class FtClient
{
    ...
    private UrlShortenerUtility $utility;

    public function __construct(UrlShortenerUtility $utility, array $options)
    {
    ...
    public function get_utility(): UrlShortenerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UrlShortenerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UrlShortenerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UrlShortenerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UrlShortenerConfig::make_feature($name);
    ...
    private static function buildUrl(UrlShortenerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UrlShortenerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UrlShortenerResponse($fetched);
        }
    ...
    private function fail(UrlShortenerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UrlShortenerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
