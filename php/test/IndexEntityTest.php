...
class IndexEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set URLSHORTENER_TEST_INDEX_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function index_basic_setup($extra)
{
    ...
    $client = UrlShortenerSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("URLSHORTENER_TEST_INDEX_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "URLSHORTENER_TEST_INDEX_ENTID" => $idmap,
        "URLSHORTENER_TEST_LIVE" => "FALSE",
        "URLSHORTENER_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["URLSHORTENER_TEST_INDEX_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["URLSHORTENER_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new UrlShortenerSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["URLSHORTENER_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
