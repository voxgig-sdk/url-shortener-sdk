...
import (
	...
	omni "github.com/voxgig-sdk/url-shortener-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UrlShortenerError); is {
				out := map[string]any{
					"name":    "UrlShortenerError",
...
func omniCtx(arg any, client *sdk.UrlShortenerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UrlShortenerSDK); is {
			client = live
