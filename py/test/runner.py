...
class UrlShortenerTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UrlShortenerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UrlShortenerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UrlShortenerTestRunner.getenv("URL_SHORTENER_TEST_LIVE")
        override = UrlShortenerTestRunner.getenv("URL_SHORTENER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UrlShortenerTestRunner.getenv(key)
        ...
        explain = UrlShortenerTestRunner.getenv("URL_SHORTENER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UrlShortenerTestRunner._test_control is not None:
            return UrlShortenerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UrlShortenerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UrlShortenerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UrlShortenerTestRunner.load_test_control()
...
def load_env_local():
    UrlShortenerTestRunner.load_env_local()


def env_override(m):
    return UrlShortenerTestRunner.env_override(m)


def entity_data(v):
    return UrlShortenerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UrlShortenerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UrlShortenerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UrlShortenerTestRunner.load_test_control()


def live_client_options():
    return UrlShortenerTestRunner.live_client_options()


def live_delay_ms():
    return UrlShortenerTestRunner.live_delay_ms()
