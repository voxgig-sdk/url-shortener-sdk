...
// UrlShortener SDK retry feature
...
class UrlShortenerRetryFeature extends UrlShortenerBaseFeature
{
    ...
    public function init(UrlShortenerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UrlShortenerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UrlShortenerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UrlShortenerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
