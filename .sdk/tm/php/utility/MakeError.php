...
// UrlShortener SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class UrlShortenerMakeError
{
    public static function call(?UrlShortenerContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UrlShortenerContext([], null);
        }
        $op = $ctx->op ?? new UrlShortenerOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UrlShortenerResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UrlShortenerError) ? $err->msg : (string)$err;
        $msg = "UrlShortenerSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new UrlShortenerError('', $msg, $ctx);
