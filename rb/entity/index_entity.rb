...
class IndexEntity
  ...
  def data_set(args)
    if args
      @_data = UrlShortenerHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = UrlShortenerHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Index, Hash] the loaded Index; raises UrlShortenerError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UrlShortenerHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
