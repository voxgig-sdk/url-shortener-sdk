...
require_relative 'UrlShortener_types'


class UrlShortenerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UrlShortenerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UrlShortenerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UrlShortenerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UrlShortenerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UrlShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UrlShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UrlShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => UrlShortenerError.new(
        "#{op}_allow",
        "UrlShortenerSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UrlShortenerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UrlShortenerError.new(
        "graphql_error", "UrlShortenerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UrlShortenerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
