...
module UrlShortenerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UrlShortenerResponse.new({})
    ...
    if fetch_err
      ...
      response = UrlShortenerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UrlShortenerResponse.new(fetched)
    else
