...
[MCP](https://modelcontextprotocol.io) server exposing the UrlShortener SDK as
two agent tools — `url-shortener_list` and `url-shortener_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/url-shortener-mcp)
make build

# 2. Provide credentials via the environment
export URL_SHORTENER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user url-shortener \
  -- /absolute/path/to/url-shortener-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./url-shortener-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// url-shortener_list: first page of records
{ "entity": "index" }
{ "entity": "index", "query": { } }

// url-shortener_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/url-shortener-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user url-shortener \
     -- "$PWD"/dist/*/url-shortener-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `url-shortener_list` and `url-shortener_load` tools now appear
   in new sessions. Ask the agent to *"list index using url-shortener"*
   and it calls `url-shortener_list` with `{"entity":"index"}`.
...
```sh
export URL_SHORTENER_APIKEY=sk_live_xxx            # API key
export URL_SHORTENER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `url-shortener_list` tool
...
### Call the `url-shortener_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `url-shortener_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `url-shortener_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `URL_SHORTENER_APIKEY` | API key sent with every request. |
| `URL_SHORTENER_BASE` | Optional override of the API base URL. |
...
```sh
./url-shortener-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"url-shortener_load","arguments":{"entity":"index","query":{"id":1}}}}'
```
